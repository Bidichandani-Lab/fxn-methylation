...
for ((i = 0; i < ${#sorted_args[@]}; i+=2)) ; do
    ...
    if [[ "${r1_name}" != "${r2_name}" ]] ; then
        echo "R1 and R2 FASTQ files don't match (\"${r1_path}\" and \"${r2_path}\")" >&2
        exit 1
    fi
done
...
for ((i = 0; i < ${#sorted_args[@]}; i+=2)) ; do
    ...
    # Merge
    # A different merging script has to be run and used if amp1 is included because
    # there is gap between amp1 and the rest
